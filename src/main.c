...
static lib_status_t install_rootfs(const char *rootfs_path, params_t params) {
    ...
    char *download_cmd = strdup("wget -qO- https://archive.archlinux.org/iso/2025.02.01/archlinux-bootstrap-x86_64.tar.zst | tar --strip-components 1 -x --zstd -C ");
    ...
    if(
        container_context_exec_shell(cc, "echo 'Server = https://archive.archlinux.org/repos/2025/02/01/$repo/os/$arch' > /etc/pacman.d/mirrorlist") != 0
